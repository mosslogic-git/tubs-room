...
import {speedOfSound,surfaces,firstReflection,traceRay,earlyPaths,beamGain,sabine,roomModes,modalShape,treatmentPanels,calculatePointSPL,calculateSPLGrid,calculateBoundaryExposure,calculateIsolationAssemblies} from '../dist/acoustic-physics.js';
...
const source={position:[-2,1,0],beam:null,productId:'obslk',role:'top'},receiver=[2,1,0];
...
// SPL test assertions: inverse-square distance drop (~6 dB per distance doubling) and dancefloor grid
const spl1m = calculatePointSPL([-2, 1, 1], [source]);
const spl2m = calculatePointSPL([-2, 1, 2], [source]);
near(spl1m, 104, 0.01);
near(spl1m - spl2m, 20 * Math.log10(2), 0.01);
const grid = calculateSPLGrid(room, [source], 12, 16);
assert.equal(grid.values.length, 12 * 16);
// Sound isolation tests: boundary acoustic exposure & engineered assembly specs
const exposure = calculateBoundaryExposure(room, [source]);
assert.ok(exposure.peakSPL > 85, 'Peak SPL on boundary must exceed 85 dB');
assert.ok(exposure.boundaries.floor.spl > 80, 'Floor boundary exposure must exceed 80 dB');
const iso = calculateIsolationAssemblies(room, [source], 45);
assert.ok(iso.reqTL >= 40, `Required TL must be >= 40 dB, got ${iso.reqTL}`);
assert.ok(iso.floatingFloor.totalPucks > 50, `Floating floor pucks must be > 50, got ${iso.floatingFloor.totalPucks}`);
assert.ok(iso.floatingFloor.floorResonanceHz < 20, 'Floating floor natural resonance must be below audible 20 Hz');
assert.equal(iso.cornerTraps.count, 4, 'Corner traps count must be 4');
assert.ok(iso.soundLock.attenuationDb >= 20, 'Sound lock must provide at least 20 dB attenuation');
...
console.log('PASS: analytical image paths, inverse-square energy, reflection direction/loss, full absorption, beam angles, modal frequencies/nodes, Sabine treatment, SPL calculations, and '+cases+' cabinet bounds.');
