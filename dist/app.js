...
function update(){
 state.focus=null;document.body.classList.remove('focused');$('focus-info').hidden=true;
...
function toggleDetails(open){$('details').hidden=!open;$('details-btn').setAttribute('aria-expanded',String(open));if(open)$('close-details').focus();else $('details-btn').focus();}
...
$('reset').addEventListener('click',()=>{Object.assign(state,{width:8,depth:10,height:3.5,model:'auto',cluster:false});$('model').value='auto';$('layout').checked=false;update();setView('inside');});
$('reset-view').addEventListener('click',()=>{clearFocus();setView('inside');});document.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>setView(b.dataset.view)));
...
function clearFocus(){state.focus=null;$('focus-info').hidden=true;document.body.classList.remove('focused');syncNavigation(configuration(state.width*state.depth));yaw=pitch=0;}
function focusProduct(product){if(!$('details').hidden)toggleDetails(false);state.focus=product.id;setView('inside',true);$('focus-info').hidden=false;$('focus-title').textContent=product.name;$('focus-desc').textContent=`${product.count} × ${product.description.toLowerCase()}`;$('product-link').href=`https://tubs-audio-nz.netlify.app/products/${product.id}`;document.body.classList.add('focused');syncNavigation(configuration(state.width*state.depth));}
function setView(view,retainFocus=false){
 state.view=view;yaw=pitch=0;if(!retainFocus)clearFocus();
 ...
 $('look-hint').textContent=view==='inside'?'Drag to look around.':'Drag to orbit. Scroll to zoom.';
...
function createCabinet(item){
 ...
 return {object:shell,position:new THREE.Vector3(),scale:1,key:`${item.index}-${item.productId}`,role:item.role};
}
function arrange(){
...
async function init(){
 ...
 camera=new THREE.PerspectiveCamera(65,1,.08,150);camera.position.set(0,1.65,3.8);controls=new OrbitControls(camera,renderer.domElement);controls.enabled=false;controls.enableDamping=!reduced;controls.minDistance=5;controls.maxDistance=90;controls.maxPolarAngle=Math.PI/2-.025;controls.enablePan=false;
 ...
 const vp=$('viewport');vp.addEventListener('pointerdown',e=>{if(state.view!=='inside')return;drag={x:e.clientX,y:e.clientY,yaw,pitch};vp.setPointerCapture(e.pointerId);});vp.addEventListener('pointermove',e=>{if(!drag)return;yaw=THREE.MathUtils.clamp(drag.yaw-(e.clientX-drag.x)*.002,-.65,.65);pitch=THREE.MathUtils.clamp(drag.pitch+(e.clientY-drag.y)*.0015,-.25,.35);});const stop=()=>drag=null;vp.addEventListener('pointerup',stop);vp.addEventListener('pointercancel',stop);vp.addEventListener('lostpointercapture',stop);
 vp.addEventListener('keydown',e=>{if(state.view!=='inside')return;if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();yaw=THREE.MathUtils.clamp(yaw+(e.key==='ArrowLeft'?.06:e.key==='ArrowRight'?-.06:0),-.65,.65);pitch=THREE.MathUtils.clamp(pitch+(e.key==='ArrowUp'?.04:e.key==='ArrowDown'?-.04:0),-.25,.35);}});
 applyLighting();
 ...
 if(state.view==='inside'){
  const config=configuration(state.width*state.depth),product=config.products.find(p=>p.id===state.focus),subject=product?speakers.find(s=>s.role===product.role&&s.scale===1):null;
  if(subject){const pos=subject.object.position;const distance=camera.aspect<.7?2.9:2.1;nextCam.set(pos.x+.55,Math.max(1.3,pos.y+.8),pos.z+distance);nextLook.set(pos.x,pos.y+.55,pos.z);}
  else {nextCam.set(0,Math.min(1.65,currentSize.y*.6),currentSize.z*.36);nextLook.set(0,Math.min(1.35,currentSize.y*.45),-currentSize.z/2);}
  const distance=nextCam.distanceTo(nextLook);nextLook.x+=Math.sin(yaw)*distance;nextLook.y+=Math.sin(pitch)*distance;camera.position.lerp(nextCam,t);look.lerp(nextLook,t);camera.lookAt(look);
  const fov=camera.aspect<.8?Math.min(108,2*Math.atan(Math.tan(THREE.MathUtils.degToRad(65/2))/.8*1.05/camera.aspect)*180/Math.PI):65;camera.fov=THREE.MathUtils.lerp(camera.fov,fov,t);camera.updateProjectionMatrix();
